...
seastar::future<> UdpGroup::send_to_all(UdpPacket &packet) {
  ...
  seastar::future<> await_all =
      seastar::when_all_succeed(cached_batch.begin(), cached_batch.end());

  cached_batch.clear();

  // TODO ownership problem here - if these futures try to resolve themselves
  // during a shutdown and catch an exception, they'd reference garbage memory.
  // Need to put the semaphore into a shared ptr of sorts
  // or need to relearn enable_shared_from_this?

  try {
    co_await std::move(await_all);
    outstanding_batch_tracker.signal(1);
    co_return;
  } catch (...) {
    outstanding_batch_tracker.broken();
    throw;
  }
}
